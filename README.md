...
## 右クリックで画像を保存（PCのChrome / Edge のみ）

画像を右クリックすると、原寸（`?name=orig`）を取得して `リファレンス` フォルダに直接書き込む。
複数枚の投稿は全部保存する（`<handle>_<tweetId>.jpg`、2枚目以降は `_2`, `_3`…）。

ブラウザはパスを指定して勝手に書き込めないので、**初回だけ右上の「保存先」からフォルダを選ぶ**。
選んだハンドルは IndexedDB に記憶する。フォルダ名が `リファレンス` でなければ受け付けない。

- 保存した投稿は枠と「保存済」バッジが付く（localStorage）
- **Shift+右クリック** でブラウザ標準のメニューが出る
- 非対応ブラウザ（スマホ・Firefox・Safari）では標準メニューのまま、その旨を通知する
- 失敗時は理由をトーストで出す（未設定 / 権限拒否 / 取得失敗 / 書き込み失敗）
